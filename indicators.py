...
parser.add_argument('--input_images_path', default='./input_images')
parser.add_argument('--image2smiles2image_save_path',
...
class DataLoaderVal(Dataset):
    def __init__(self, target_transform=None):
        ...
        self.clean_filenames = [os.path.join(
            gt_dir, x) for x in clean_files if is_png_file(x)]
        self.noisy_filenames = [os.path.join(
            input_dir, x) for x in noisy_files if is_png_file(x)]
...
if __name__ == '__main__':
    ...
    for ii, data_test in enumerate(tqdm(test_loader), 0):
        ...
        rgb_restored = torch.clamp(rgb_restored, 0, 1).cpu(
        ).numpy().squeeze().transpose((1, 2, 0))
        psnr_val_rgb.append(psnr_loss(rgb_restored, rgb_groundtruth))
        ssim_val_rgb.append(
            ssim_loss(rgb_restored, rgb_groundtruth, multichannel=True))
    ...
    for file in files:
        ...
        try:
            ...
            img0 = lpips.im2tensor(lpips.load_image(
                os.path.join(args.input_images_path, file)))
            img1 = lpips.im2tensor(lpips.load_image(
                os.path.join(args.image2smiles2image_save_path, file)))

            if (os.path.exists(os.path.join(args.input_images_path, file)), os.path.exists(os.path.join(args.image2smiles2image_save_path, file))):
                i = i + 1
            ...
            # MAE
            mean_absolute_error = mae(img0, img1)
    ...
    print("MAE: %f, PSNR: %f, SSIM: %f, LPIPS: %f " %
          (mean_absolute_error, psnr_val_rgb, ssim_val_rgb, average_lpips_distance))
