...
class DiffusionCLIP(object):
                ...
                if self.args.do_train:
                    if os.path.exists(save_name):
                        ...
                        for step, (x0, x_id, x_lat) in enumerate(img_lat_pairs_dic['train']):
                            ...
                            loss_clip = (
                                2 - clip_loss_func(x0, src_txt, x, trg_txt)) / 2
                            ...
                            loss = self.args.clip_loss_w * loss_clip + \
                                self.args.id_loss_w * loss_id + self.args.l1_loss_w * loss_l1
                            loss.backward()
                        ...
                        for step, (x0, _, x_lat) in enumerate(img_lat_pairs_dic['train']):
                            ...
                            with tqdm(total=len(seq_train), desc=f"CLIP iteration") as progress_bar:
                                for t_it, (i, j) in enumerate(zip(reversed(seq_train), reversed(seq_train_next))):
                                    ...
                                    loss_clip = - \
                                        torch.log(
                                            (2 - clip_loss_func(x0, src_txt, x0_t, trg_txt)) / 2)
                                    ...
                                    for p in model.module.parameters():
                                        p.grad = None
                                    print(
                                        f"CLIP {step}-{it_out}: loss_clip: {loss_clip:.3f}")
                                    # break
    ...
    def clip_latent_optim(self):
        ...
        img = torch.from_numpy(img).type(
            torch.FloatTensor).permute(2, 0, 1).unsqueeze(dim=0)
        ...
        for txt_idx, (src_txt, trg_txt) in enumerate(zip(self.src_txts, self.trg_txts)):
            ...
            optimizer = torch.optim.Adam(
                [x_lat], weight_decay=0, lr=self.args.lr_clip_lat_opt)
            clip_loss_func.target_direction = None
            ...
            with torch.set_grad_enabled(True):
                for it in range(self.args.n_iter):
                    ...
                    loss.backward()
                    print(
                        f"CLIP opt: loss_clip: {loss_clip:.3f}, loss_id: {loss_id:.3f}, loss_l1: {loss_l1:.3f}")
                    tvu.save_image((x + 1) * 0.5, os.path.join(self.args.image_folder,
    ...
    def edit_images_from_dataset(self):
        ...
        for mode in ['test']:
            ...
            if os.path.exists(pairs_path):
                ...
                for step, (x0, x_id, e_id) in enumerate(img_lat_pairs_dic[mode]):
                    tvu.save_image(
                        (x0 + 1) * 0.5, os.path.join(self.args.image_folder, f'{mode}_{step}_0_orig.png'))
                    tvu.save_image(
                        (x_id + 1) * 0.5, os.path.join(self.args.image_folder, f'{mode}_{step}_1_rec.png'))
                    if step == self.args.n_precomp_img - 1:
                        break
                continue
            else:
            ...
            for step, img in enumerate(loader):
                ...
                tvu.save_image(
                    (x + 1) * 0.5, os.path.join(self.args.image_folder, f'{mode}_{step}_1_rec.png'))
                if step == self.args.n_precomp_img - 1:
                    break
        ...
        with torch.no_grad():
            ...
            for it in range(self.args.n_iter):
                ...
                x = x0 * a[self.args.t_0 - 1].sqrt() + e * \
                    (1.0 - a[self.args.t_0 - 1]).sqrt()
                tvu.save_image(
                    (x + 1) * 0.5, os.path.join(self.args.image_folder, f'1_lat.png'))
