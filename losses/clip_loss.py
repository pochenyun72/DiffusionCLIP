...
class CLIPLoss(torch.nn.Module):
    ...
    def compute_img2img_direction(self, source_images: torch.Tensor, target_images: list) -> torch.Tensor:
        with torch.no_grad():
            ...
            for target_img in target_images:
                preprocessed = self.clip_preprocess(
                    Image.open(target_img)).unsqueeze(0).to(self.device)
    ...
    def set_text_features(self, source_class: str, target_class: str) -> None:
        source_features = self.get_text_features(
            source_class).mean(axis=0, keepdim=True)
        self.src_text_features = source_features / \
            source_features.norm(dim=-1, keepdim=True)

        target_features = self.get_text_features(
            target_class).mean(axis=0, keepdim=True)
        self.target_text_features = target_features / \
    ...
    def clip_angle_loss(self, src_img: torch.Tensor, source_class: str, target_img: torch.Tensor, target_class: str) -> torch.Tensor:
        ...
        cos_text_angle = cos_text_angle.unsqueeze(
            0).repeat(img_angle.size()[0], 1, 1)
    ...
    def clip_directional_loss(self, src_img: torch.Tensor, source_class: str, target_img: torch.Tensor, target_class: str) -> torch.Tensor:
        ...
        edit_direction /= (edit_direction.clone().norm(dim=-
                           1, keepdim=True) + 1e-7)
    ...
    def generate_patches(self, img: torch.Tensor, patch_centers, size):
        ...
        for batch_idx in range(batch_size):
            for patch_idx in range(num_patches):
                ...
                center_x = patch_centers[batch_idx *
                                         num_patches + patch_idx][0]
                center_y = patch_centers[batch_idx *
                                         num_patches + patch_idx][1]

                patch = img[batch_idx:batch_idx+1, :, center_y - half_size:center_y +
                            half_size, center_x - half_size:center_x + half_size]
    ...
    def clip_patch_similarity(self, src_img: torch.Tensor, source_class: str, target_img: torch.Tensor, target_class: str) -> torch.Tensor:
        ...
        src_scores = self.patch_scores(
            src_img, source_class, patch_centers, patch_size)
    ...
    def patch_directional_loss(self, src_img: torch.Tensor, source_class: str, target_img: torch.Tensor, target_class: str) -> torch.Tensor:
        ...
        if self.patch_text_directions is None:
            ...
            self.patch_text_directions = torch.cat([self.compute_text_direction(
                pair[0], pair[1]) for pair in parts_classes], dim=0)
        ...
        patch_class_scores = cosine_dists * \
            (edit_direction @ self.patch_text_directions.T).softmax(dim=-1)
    ...
    def forward(self, src_img: torch.Tensor, source_class: str, target_img: torch.Tensor, target_class: str, texture_image: torch.Tensor = None):
        ...
        if self.lambda_global:
            clip_loss += self.lambda_global * \
                self.global_clip_loss(target_img, [f"a {target_class}"])

        if self.lambda_patch:
            clip_loss += self.lambda_patch * \
                self.patch_directional_loss(
                    src_img, source_class, target_img, target_class)

        if self.lambda_direction:
            clip_loss += self.lambda_direction * \
                self.clip_directional_loss(
                    src_img, source_class, target_img, target_class)

        if self.lambda_manifold:
            clip_loss += self.lambda_manifold * \
                self.clip_angle_loss(src_img, source_class,
                                     target_img, target_class)

        if self.lambda_texture and (texture_image is not None):
            clip_loss += self.lambda_texture * \
                self.cnn_feature_loss(texture_image, target_img)
